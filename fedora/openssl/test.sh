...
test_openssl_functionality() {
    ...
    # Create a test file
    echo "OpenSSL test message" | sudo tee "$test_file" >/dev/null

    # Encrypt the file
    sudo openssl enc -aes-256-cbc -salt -in "$test_file" -out "$encrypted_file" -pass pass:testpassword

    # Decrypt the file
    sudo openssl enc -d -aes-256-cbc -in "$encrypted_file" -out "$decrypted_file" -pass pass:testpassword

    # Check if the decrypted content matches the original
    if sudo diff "$test_file" "$decrypted_file" >/dev/null; then
...
PACKAGE_VERSION=$(sudo openssl version | awk '{print $2}')
