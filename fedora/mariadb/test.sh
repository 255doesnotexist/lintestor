...
set_root_password() {
    sudo mysql -e "ALTER USER 'root'@'localhost' IDENTIFIED BY 'YourStrongPassword';"
...
test_mariadb_service() {
    ...
    local mysql_status=$(mysqladmin ping --host=localhost --user=root --password=YourStrongPassword 2>/dev/null)
