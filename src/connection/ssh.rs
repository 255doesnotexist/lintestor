...
impl SSHConnectionManager {
    ...
    pub fn new(connection_config: &ConnectionConfig) -> Result<Self> {
        ...
        if let Some(jumps) = jump_hosts {
            if !jumps.is_empty() {
                ...
                // 身份验证
                Self::authenticate_session(&mut session, username, password, private_key_path, public_key_path)?;
    ...
    fn setup_ssh_proxy_with_jumphosts(
        ...
        for retry in 0..max_retries {
            ...
            cmd.arg("-N")  // 不执行远程命令
               ...
               .arg("-o").arg("BatchMode=yes")  // 不询问密码（使用SSH配置和密钥）
    ...
    /// 使用密钥文件进行认证
    fn authenticate_with_key(session: &mut Session, username: &str, private_key_path: &str) -> Result<()> {
        ...
        prikey_file.read_to_end(&mut prikey_contents)
        ...
        let res = session.userauth_pubkey_memory(username, None, &String::from_utf8_lossy(&prikey_contents), None);
    ...
    /// 带重试的连接方法
    fn connect_with_retry<F>(connect_fn: F, max_retries: usize, timeout_secs: u64, error_message: &str) -> Result<TcpStream>
    where
