...
pub fn aggregate_reports_from_dir(reports_dir: Option<&Path>, output_path: Option<&Path>) -> Result<()> {
    ...
    if !reports_dir.exists() || !reports_dir.is_dir() {
        warn!("Report directory does not exist or is not a directory: {}", reports_dir.display());
        return Ok(());
    }
    ...
    if all_reports.is_empty() {
        warn!("No report files found in directory: {}", reports_dir.display());
        return Ok(());
    }
    ...
    info!("Successfully aggregated {} reports to {}", all_reports.len(), output_path.display());
...
fn extract_report_metadata(content: &str, file_path: &Path) -> Result<serde_json::Value> {
    // 提取基本信息
    let template_id = file_path.file_stem()
        .and_then(|s| s.to_str())
        .unwrap_or("unknown")
        .replace(".report", "");
    ...
    {
        title = title_match.get(1).map_or("Untitled Test", |m| m.as_str()).to_string();
    }
    ...
    if let Some(unit_match) = regex::Regex::new(r#"unit_name:\s*"?(.*?)"?[\r\n]"#)
        ...
        .and_then(|re| re.captures(content))
    {
        unit_name = unit_match.get(1).map_or("unknown", |m| m.as_str()).trim().to_string();
    }
